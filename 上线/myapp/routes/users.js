...
router.get('/getNotice',(req,res)=>{
  var s_std_id = req.query.id;
  var sql = $sql.user.getNotice;
  conn.query(sql, [s_std_id], function(err,result){
    if(err){
      console.log(err);
    }
    if(result){
...
router.post("/changeResume",(req,res)=>{
  var s_std_id = req.body.id;
  var changeData = req.body.resume_content;
  var sql = $sql.user.changeResume;
  conn.query(sql, [changeData, s_std_id], (err,result) => {
    if(err){
      console.log(err);
    }
    if(result){
